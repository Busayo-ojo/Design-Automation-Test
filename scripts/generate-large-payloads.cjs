...
console.log(`Remaining icons count to extract starting from index 225: ${remaining.length}`);
...
batches.forEach((batch, idx) => {
  const batchNum = idx + 16; // Save outputs as figma-output-batch-16, 17, ...
  const code = `
${headersStr}
...
return results;
`;
  fs.writeFileSync(path.join(outDir, `batch-15-${batchNum}.js`), code.trim(), 'utf8');
});

console.log('15-sized batch payloads generated.');
